...
import com.example.commons.core.utils.config.modelmapper.jdk8.Jdk8Module;
import com.example.commons.core.utils.config.modelmapper.json.FastJsonModule;
import com.example.commons.core.utils.config.modelmapper.jsr310.Jsr310Module;
import com.example.commons.core.utils.config.modelmapper.jsr310.Jsr310ModuleConfig;
...
public class BeanConvert {
    ...
    /**
     * 列表转换 使用stream并行流 数据量大时使用
     *
     * @param list        源列表数据
     * @param targetClass 目标对象类型
     * @return 目标类型集合
     */
    public static <T> List<T> listConvertParallel(List<?> list, Class<T> targetClass) {
        if (CollUtil.isEmpty(list)) {
            return Collections.emptyList();
        }
        return list.parallelStream().map(obj -> convert(obj, targetClass)).collect(Collectors.toList());
    }
